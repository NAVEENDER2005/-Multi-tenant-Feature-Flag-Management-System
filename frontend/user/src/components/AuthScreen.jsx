...
export default function AuthScreen() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data.user.role !== 'end_user') { setError('This portal is for end users only.'); return }
  ...
  const handleSignup = async (e) => {
    ...
    try {
      await apiFetch('/api/auth/signup', {
        method: 'POST',
        body: { email, password, role: 'end_user', organization_id: orgId },
      })
      const data = await apiFetch('/api/auth/login', { method: 'POST', body: { email, password } })
      login(data.token, data.user)
    } catch (err) {
    ...
    <div className="center-wrap">
      <div className="auth-card">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          ...
          <span className="role-badge" style={{ background: 'rgba(16,185,129,.15)', color: 'var(--success)', borderColor: 'var(--success)' }}>End User</span>
        </div>
        ...
        {tab === 'login' ? (
          ...
          <form onSubmit={handleSignup} noValidate>
            ...
            <div className="form-group" style={{ marginBottom: '1.25rem' }}>
              <label>Role</label>
              <input type="text" value="End User" disabled style={{ opacity: .6 }} />
            </div>
